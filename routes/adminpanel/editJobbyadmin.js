...
router.post('/', async (req, res) => {
  try {
    ...
    if (!validation.error) {
      let query = ` update jobs set subname = "admin" ,  subname = "${req.body.subname}" ,  rating = ${req.body.rating} , distance = ${req.body.distance} , price = ${req.body.city} ,  lng = "${req.body.lng}" , lat = "${req.body.lat}"  where id = ${req.body.jobid}  `;
      console.log(query)
